...
g = 9.81
R = 85
eps_x = 0.00000001    # The maximal error is stated in the assignment!
eps_f = 0.00000001
kMax = 100
# First task: solution(s) for R=85.0 (m).
theta, err, res, conv = bis(F,0,(np.pi)/2,eps_x,eps_f,kMax)
if conv:
    print("First solution: R=%f, theta=%f +/- %f." % (R,theta,err))
else:
    print("Bisection did not converge!")
theta, err, res, conv = bis(F,(np.pi)/2,(np.pi),eps_x,eps_f,kMax)
if conv:
